...
// It is used to search for hotels that have a room with a private bathroom.
// It overrides the search method and filters out hotels that do not have a room with a private bathroom. 
//It then sorts the remaining hotels based on the sort strategy provided.
import java.util.List;
import java.util.Scanner;

public class SearchByBathRoom  implements SearchStrategy {
    @Override
    public List<Hotel> search(List<Hotel> filters, SortStrategy sortStrategy) {
        ...
        for(int i=0;i<filters.size();i++){
            if(filters.get(i).hasRoomType(str) == false||filters.get(i).rooms.get(str).get(0).hasPrivateBathroom() == false){
                filters.remove(i);
